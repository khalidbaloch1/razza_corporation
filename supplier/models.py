...
class Supplier(models.Model):
        ...
        def ledger_balance(self):
            customer_ledgers = self.supplier_ledger.all()
        ...
        def total_payment(self):
            purchases = self.purchase_supplier.all()
            if purchases.exists():
                total_payments = purchases.aggregate(Sum('advance_payment'))
                total_advance_payment = float(total_payments.get('advance_payment__sum'))
            return self.ledger_payment_balance() + total_advance_payment

        def remaining_balance(self):
            return self.ledger_payment_balance() - self.ledger_balance()


class SupplierLedger(models.Model):
    supplier = models.ForeignKey(Supplier, on_delete=models.CASCADE, related_name='supplier_ledger')
...
class SupplierCredit(models.Model):
    supplier = models.ForeignKey(Supplier, on_delete=models.CASCADE, related_name='credit_ledger')
